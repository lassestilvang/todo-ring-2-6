...
export default function TaskDetailScreen({ route, navigation }) {
  ...
  const fetchTask = async () => {
    try {
      const res = await fetch(`http://localhost:3000/api/tasks?id=${taskId}`);
  ...
  const fetchSubtasks = async () => {
    try {
      const res = await fetch(`http://localhost:3000/api/subtasks?taskId=${taskId}`);
      const json = await res.json();
  ...
  const toggleSubtask = async (id, isCompleted) => {
    try {
      await fetch(`http://localhost:3000/api/subtasks?id=${id}`, {
        method: 'PUT',
      });
