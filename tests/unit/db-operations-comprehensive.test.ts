/**
 * Database Operations - Comprehensive Behavior Tests
 *
 * Tests the actual behavior of database operations functions with proper mocking.
 */

import { describe, it, expect, beforeEach, afterEach, vi } from 'vitest';

// Create mock database that tracks calls
    ...
    prepare: vi.fn((sql: string) => {
      const statement: any = {
        ...
        run: vi.fn((...params: any[]) => {
          ...
          statement._runParams = params;
          return { lastInsertRowid: params[0] || 'test-id' };
        }),
        get: vi.fn((...params: any[]) => {
          calls.push({ type: 'get', sql, params });
          statement._getParams = params;
          return statement._mockResult || null;
        }),
        all: vi.fn((...params: any[]) => {
          calls.push({ type: 'all', sql, params });
          statement._allParams = params;
          return statement._mockResult || [];
        }),
      };
      return statement;
    }),
