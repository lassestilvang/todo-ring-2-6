...
describe('Validation Schemas - Comprehensive', () => {
  ...
  describe('validateSchema', () => {
    it('should return parsed data on success', () => {
      ...
      const result = validateSchema(TaskSchema, data);
      expect(result.title).toBe('Test');
    });

    it('should throw error on validation failure', () => {
      expect(() => validateSchema(TaskSchema, { title: '' })).toThrow();
    });

    it('should throw error with message details', () => {
      try {
        validateSchema(TaskSchema, { title: '' });
